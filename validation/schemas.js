...
export const TRIP_STATUSES = ["CREATED", "ASSIGNED", "PICKED", "DELIVERED", "INVOICED", "PAID"];
...
export const GST_TYPES = ["IGST", "CGST_SGST"];
...
export const orderUpdateSchema = Object.fromEntries(Object.entries(orderCreateSchema).map(([k, v]) => [k, { ...v, required: false }]));
...
export const invoiceUpdateSchema = Object.fromEntries(Object.entries(invoiceCreateSchema).map(([k, v]) => [k, { ...v, required: false }]));
export const expenseCreateSchema = {
    category: { type: "string", required: true },
    date: { type: "iso-date", required: true },
    amount: { type: "number", required: true, min: 0 },
    paymentMode: { type: "string", required: true, enum: ["CASH", "BANK", "UPI", "CHEQUE"] },
