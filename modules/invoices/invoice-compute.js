...
export function computeInvoiceTotals(input) {
    const subTotal = r2(input.orders.reduce((sum, o) => {
...
export function summarisePaidAmount(payments = []) {
    return r2(payments.reduce((s, p) => s + (Number(p.amount) || 0), 0));
}
export function deriveInvoiceStatus(totals, paidAmount, dueDate, currentStatus) {
    ...
    if (paidAmount > 0)
        return "PARTIAL";
    if (dueDate && Date.parse(dueDate) < Date.now())
        return "OVERDUE";
