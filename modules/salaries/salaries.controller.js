...
function paymentModeToExpenseMode(mode) {
    return mode === "CASH" ? "CASH" : mode === "UPI" ? "UPI" : mode === "CHEQUE" ? "CHEQUE" : "BANK";
}
const today = () => new Date().toISOString().split("T")[0];
export async function listDriverSalaries(req, res) {
    try {
...
export async function createDriverSalary(req, res) {
    try {
...
export async function listEmployeeSalaries(req, res) {
    try {
...
export async function createEmployeeSalary(req, res) {
    try {
        const userId = req.user.id;
