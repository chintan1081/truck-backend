 ...
 */
function blankForColumnType(type: unknown): unknown {
  if (
    type === Number ||
 ...
 */
export function applyColumnDefaults<T extends ObjectLiteral>(
  repo: Repository<T>,
  data: Record<string, unknown>
): Record<string, unknown> {
  for (const col of repo.metadata.columns) {
    if (col.isPrimary || col.isGenerated || col.isNullable) continue;
